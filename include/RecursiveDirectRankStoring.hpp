...
class RecursiveDirectRankStoringMmphf {
    private:
        ...
        struct TreeNode {
            ...
            TreeNode (const char *ptr, size_t length) {
                ...
                if (length == 1 * sizeof(uint8_t) + 2 * sizeof(uint16_t) + 1 * sizeof(uint32_t)) {
            ...
            size_t rawSpaceUsage() const {
                if (strategy == CHUNK_DRS) {
                    return 1 * sizeof(uint8_t) + 2 * sizeof(uint16_t) + 1 * sizeof(uint32_t);
                } else {
        ...
        explicit RecursiveDirectRankStoringMmphf(const std::vector<std::string> &strings) {
            ...
            if (treeNodesInput.size() == 1) {
                ...
                for (size_t i = 0; i < treeNodesInput.size(); i++) {
                    ...
                    } else if (test.strategy == BucketStrategy::CHUNK_DRS) {
    ...
    public:
        ...
        uint64_t operator ()(const std::string &string) {
            ...
            size_t offset = treeNodeDataOffsets.move(treeNodeIndex).second;
            size_t nextOffset = treeNodeDataOffsets.move(treeNodeIndex + 1).second;
            ...
            while (true) {
                ...
                if (bucketSize < DIRECT_RANK_STORING_THRESHOLD) {
                    ...
                    size_t nodeRawPtrOffset = treeNodeDataOffsets.move(treeNodeIndex).second;
                    size_t nextNodeRawPtrOffset = treeNodeDataOffsets.move(treeNodeIndex + 1).second;
